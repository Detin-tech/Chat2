					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} • Prosper Chat`, {
								body: content,
								icon: `${WEBUI_BASE_URL}/static/static/favicon.png`
							});
						}
					}
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${data?.user?.name} (#${event?.channel?.name}) • Prosper Chat`, {
							body: data?.content,
							icon: data?.user?.profile_image_url ?? `${WEBUI_BASE_URL}/static/static/favicon.png`
						});
					}
				}
...
<svelte:head>
	...
	<link crossorigin="anonymous" rel="icon" href="{WEBUI_BASE_URL}/static/static/favicon.png" />
</svelte:head>
